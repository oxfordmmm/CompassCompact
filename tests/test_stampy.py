...
3. Copy expected basecall output fasta to tests/data/test_output/expected_output
    python3 tests/test_stampy.py bam -profile test_docker (under CompassCompact, test bam input)
    python3 tests/test_stampy.py fastq -profile test_docker (under CompassCompact, test fastq input)
