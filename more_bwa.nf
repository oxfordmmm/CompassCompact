...
process pick_reference {
    ...
    output:
    set dataset_id, file("${dataset_id}.clean.1.fq.gz"), file("${dataset_id}.clean.2.fq.gz"), stdout into bwa
    ...
    if ref_dir.is_dir():
        sys.stdout.write(str(ref_dir) + '/')
    else:
        sys.stdout.write('failed')

    sys.stderr.write('species found: ' + str(species) + '\\n')
    sys.stderr.write('top species: ' + str(top_species) + '\\n')
    sys.stderr.write('reference directory: ' + str(ref_dir) + '\\n')
    sys.stderr.write('reference directory exists: ' + str(ref_dir.is_dir()) + '\\n')
...
process bwa {
    ...
    input:
    set dataset_id, file("${dataset_id}.clean.1.fq.gz"), file("${dataset_id}.clean.2.fq.gz"), ref_dir from bwa
    when:
    ref_dir != "failed"

    output:
    set dataset_id, file("${dataset_id}_alignment.sam"), ref_dir into bwa_map

    """
    cp ${ref_dir}/ref.fa .
    $BWA/bwa index ref.fa
    $BWA/bwa mem -R '@RG\tID:${dataset_id}\tSM:null\tLB:null\tCN:null' -t 1 ${ref_dir}/ref.fa "${dataset_id}.clean.1.fq.gz" "${dataset_id}.clean.2.fq.gz" > ${dataset_id}_alignment.sam
    """
}
...
process annotvcf {
    ...
    input:
    set dataset_id, file("${dataset_id}.out.vcf"), file("${dataset_id}.pileup.vcf"), ref_dir from snpcalling

    output:
    set dataset_id, file("${dataset_id}.annotvcf.vcf"), ref_dir into annotcvf

    """
    # get reference name. This assumes it's in a certain format e.g.
    # '/data/references/clockwork/qc_vc/Reference.NC_000962.3//ref.fa'
    ...
    -refmask /data/references/compass/mask/\${REF_ID}_repmask.array \
